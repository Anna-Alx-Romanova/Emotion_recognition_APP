...
def process_image(
    ...
    for face_img, box in zip(face_images, boxes):
        ...
        draw.rectangle(
            (x_min, y_min, x_min + text_width + padding, y_min + text_height + padding),
            fill=color,
        )
...
def emo_predict(filename):
    model = load_model("models/model_10.pt")
    ...
    predict_emotion = FacePredictor(
        model, emotions=sorted(list(EMOTION_COLOR_MAP.keys()))
    )
    ...
    result_analysis = pil_img.save(filename)


def sentiment_analysis(filename):
    loaded_image1 = os.path.join(app.config["IMAGE_UPLOADS"], filename)
    emo_predict(loaded_image1)
